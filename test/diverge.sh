#!/usr/bin/env bash
# Multi-mode test gate — tracks the LATEST aql from `main`.
#
# AQL is on an iterative-improvement track, so this gate always targets the
# newest `aql` and reports current status rather than pinning a "best" build.
# It runs every suite under all three execution modes:
#
#   * INTERPRETER  — `aql suite.aql`            (the supported path)
#   * CHECK        — `aql check suite.aql`      (static type-checker)
#   * BYTECODE     — `aql --force-compile suite.aql`
...
#
# Resolving the aql build (latest main by default):
#   1. $BYTECODE_AQL, if set and it accepts --force-compile;
#   2. else build $BYTECODE_AQL_REF (default: current `main` HEAD), cached by
...
#      new sha);
#   3. else (offline) the on-PATH `aql`, or the newest cached build, if either
#      accepts --force-compile.
#
# Run from anywhere:
#   bash test/diverge.sh
#   BYTECODE_AQL=/path/to/aql bash test/diverge.sh     # use a specific binary
#   BYTECODE_AQL_REF=<sha>     bash test/diverge.sh     # pin a specific commit
set -uo pipefail

cd "$(dirname "$0")/.."   # repo root, so `import "./decision.aql"` resolves

BYTECODE_AQL_REF="${BYTECODE_AQL_REF:-}"   # empty => resolve latest main HEAD
API_MAIN="https://api.github.com/repos/aql-lang/aql/commits/main"
TARBALL="https://codeload.github.com/aql-lang/aql/tar.gz"
...
# Build aql at $1 (a full sha) into a sha-named cache binary; echo its path.
build_ref() {
  local ref="$1" bin="$HOME/.local/bin/aql-bytecode-${1:0:12}"
  if supports_force_compile "$bin"; then echo "$bin"; return 0; fi
  command -v go >/dev/null 2>&1 || { red "no \`go\` toolchain; cannot build aql @ ${ref:0:12} (see docs/how-to.md)"; return 1; }
...
newest_cached_bytecode_aql() {
  local b; for b in $(ls -t "$HOME/.local/bin/aql-bytecode-"* 2>/dev/null); do
...
AQL="$(resolve_aql)" || { red "could not resolve a bytecode-capable aql (no network and no cached build)"; exit 2; }
supports_force_compile "$AQL" || { red "resolved aql rejects --force-compile: $AQL"; exit 2; }
